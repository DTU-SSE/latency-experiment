...
export const s1 = engineInstaller.designState('s1')
    ...
    .command(EngineInstallationProtocol.cmdRequestEngine, [Events.requestEngine], (ctx) => {
        ...
        return [Events.requestEngine.make({ item: engine, to: "myPosition", msgID: randomUUID() })]
    })
    .finish()
...
export const s3 = engineInstaller.designState('s3')
    ...
    .command(EngineInstallationProtocol.cmdInstallEngine, [Events.engineInstalled], (ctx) =>
        [Events.engineInstalled.make({...ctx.self, msgID: randomUUID()})])
    .finish()
...
s1.react([Events.requestEngine], s2, (ctx, event) => { return s2.make({ shape: ctx.self.shape, color: ctx.self.color, engine: event.payload.item, msgID: randomUUID() }) })
